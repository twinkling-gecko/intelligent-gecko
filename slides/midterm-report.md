...
## 画面共有によるデモンストレーションをお見せします。

<!-- (原稿)
動画によるデモンストレーションの予定でしたが、一応動作する段階まで持っていくことが出来たので、画面共有を利用してリアルタイムで進めさせていただきます。
...
ここからローカルハブであるRaspberry Piへデータを送信し、そこからプラットフォームへデータが送信され、Webアプリから統計の確認やセンサーの設定をすることが出来ます。

こちらがWebアプリのログイン画面です。
ログインするとダッシュボードに遷移し、登録済みのセンサー情報が表示されます。

左側のメニューから、センサー毎の統計情報 今回はお酒とコーヒーの残量と消費量、洗剤の月ごと消費量を表示しています。

デバイスマネジメントでは登録済みセンサーの管理や登録を行うことが出来ます。

現段階では未実装ですが、企業からのプロモーションの受信項目があります。
-->
